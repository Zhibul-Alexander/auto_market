...
export async function generateMetadata({ params }: { params: { locale: Locale; segment: string } }): Promise<Metadata> {
  ...
  const bodyLabel = t(`cars`) + ' · ' + params.segment;
  return {
    title: bodyLabel,
    description: t('segmentSub', { segment: params.segment }),
    alternates: {
      canonical: `/${params.locale}/catalog/cars/${params.segment}`,
      languages: Object.fromEntries(locales.map((l) => [l, `/${l}/catalog/cars/${params.segment}`])),
    },
  };
}
...
export default async function CarsSegmentPage({
  params,
  searchParams
}: {
  ...
  const bodyType = list?.length ? Array.from(new Set([bodyPreset, ...list])) : [bodyPreset];
  ...
  try {
    ...
    console.error('[CarsSegment] DB error:', err);
  }

  return (
    <div>
      <H2>{t('catalog.cars')} · {params.segment}</H2>
      <P style={{ marginTop: 8 }}>{t('catalog.segmentSub', { segment: params.segment })}</P>
      <div style={{ height: 16 }} />
